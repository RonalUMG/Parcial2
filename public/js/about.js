...
fetch(
  ...
  .then((data) => {
    let datosFiltrados= {
      ...
      ...data,
      certification:data.release_dates.results.filter(certification => certification.iso_3166_1=='US')
    }
    console.log('datosFiltrados: ', datosFiltrados);

    setupMovieInfo(datosFiltrados);
  });
...
const setupMovieInfo = (data) => {
  ...
  if (data.certification.length) {
    let clasificacion = data.certification[0].release_dates[0].certification
    genres.innerHTML += ` | ${clasificacion.length >0 ?clasificacion: "sin clasificacion"}`;
